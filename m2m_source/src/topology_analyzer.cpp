...
void TopologyAnalyzer::Initialize(const Parameters &par)
{
  ...
  std::vector<int> faultsIdsToRead, polygonsIdsToRead;
  ...
  if (test)
  {
    ...
    faultsIdsToRead.push_back(358);
    faultsIdsToRead.push_back(685);

    polygonsIdsToRead.push_back(920);
    polygonsIdsToRead.push_back(90);
  }

  // Set output data folder.
  converter.SetOutputFolder(par.path_output);

  // Set sill unit identifier (description contains).
  const std::string sill_descrit = getConstName(par.constNames, "SILL_STRING");
  converter.SetSillDescription(sill_descrit);

  // Reading the map data.
  converter.ReadData(par.path_faults, "LINESTRING", par.constNames, faultsIdsToRead);
  converter.ReadData(par.path_points, "POINT", par.constNames);
...
void TopologyAnalyzer::WriteUnitFaultIntersectionList(std::string &fileName,
...
void TopologyAnalyzer::WriteFaultIntersectionList(std::string &fileName,
...
void TopologyAnalyzer::AnalyzeGlobalTopology(const Parameters &par, const std::string &graph_comments)
{
  ...
  //------------------------------------------------------------------
  {
    ...
    IntersectFaultsWithFaults(converter.GetFaults(), faultIntersectionList);
  ...
  converter.IdentifyIgneousUnitContacts(unit_contacts,
                                        getConstName(par.constNames, "IGNEOUS_STRING"),
                                        getConstName(par.constNames, "VOLCANIC_STRING"));
  ...
  {// ALL contacts.
      ...
      std::string file_graph = par.path_output + "/graph_all.gml";
      graph.WriteGraph(file_graph, unit_contacts_all, "UNIT_NAME", false, 1,
  ...
  {// Stratigraphic unit contacts.
      ...
      std::string file_graph = par.path_output + "/graph_strat.gml";
      graph.WriteGraph(file_graph, unit_contacts_strat, "UNIT_NAME", false, 1,
                       par.graph_edge_width_categories, par.graph_edge_direction_type,
                       graph_comments);
  }
  ...
  {// Fault unit contacts.
      ...
      std::string file_graph = par.path_output + "/graph_fault.gml";
      graph.WriteGraph(file_graph, unit_contacts_fault, "UNIT_NAME", false, 1,
                       par.graph_edge_width_categories, par.graph_edge_direction_type,
                       graph_comments);
  }
  ...
  {// Igneous unit contacts.
      ...
      std::string file_graph = par.path_output + "/graph_ingeous.gml";
      graph.WriteGraph(file_graph, unit_contacts_ingeous, "UNIT_NAME", false, 1,
                       par.graph_edge_width_categories, par.graph_edge_direction_type,
                       graph_comments);
  }
