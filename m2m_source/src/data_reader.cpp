...
static Paths ReadCoordinates(const std::string &__line, const std::string keyword)
{
  ...
  while (true)
  {
    ...
    // Converting coordinates to integers (since the Clipper lib works with integers).
...
static size_t getFieldColumn(const std::map<std::string, size_t>& headerFields,
    ...
    const std::map<std::string, size_t>::const_iterator it = headerFields.find(fieldName);
                ...
                const std::vector<int>& idsToRead)
{
  ...
  while (std::getline(myfile, line).good())
  {
    ...
    else if (keyword == "POLYGON")
    {
      ...
      object.name = trim(ReadField(line, getFieldColumn(headerFields, fieldName)));
      ...
      object.description = ReadField(line, getFieldColumn(headerFields, fieldName));

      fieldName = getConstName(constNames, "FIELD_POLYGON_ROCKTYPE1");
      object.rocktype1 = ReadField(line, getFieldColumn(headerFields, fieldName));

      fieldName = getConstName(constNames, "FIELD_POLYGON_ROCKTYPE2");
      object.rocktype2 = ReadField(line, getFieldColumn(headerFields, fieldName));
    }
