...
class SvgBase
{
  ...
  public:
    ...
    double  y;
    unsigned fontIdx;
    TextInfo(double  _x, double  _y, unsigned _fi, const std::string& _text): text(_text), x(_x), y(_y), fontIdx(_fi) {}
  };
...
*/
  ...
  bool SaveToFile(const std::string filename, int width = 0, int height = 0, int margin = 10) const
  {
    ...
    file << svg_xml_start[0] << iwidth<< "px" << svg_xml_start[1]  << iheight << "px"
         << svg_xml_start[2] << iwidth << " " << iheight << svg_xml_start[3];

    // Flip the SVG coordinate system so that [0,0] is in the lower left instead of the upper left.
    ...
    if (!textInfos.empty() || showCoords)
    {
      if (showCoords)
      {
        ...
        for(size_t i = 0; i < polyInfos.size(); i++)
          ...
          if (polyInfos[i].si.showCoords && polyInfos[i].si.closePath) // Vitaliy changed.
          {
            for (Polygons::size_type j = 0; j < polyInfos[i].polygons.size(); ++j)
            {
            ...
            (style.polyVerticesType != 0 && polyInfos[i].si.closePath))
        {
          for (Polygons::size_type j = 0; j < polyInfos[i].polygons.size(); ++j)
          {
            for (Polygon::size_type k = 0; k < polyInfos[i].polygons[j].size(); ++k)
            {
              ...
              file << "  <circle cx=\"" << (int)((ip.X- bounds.left) * scale + margin) <<
                             "\" cy=\"" << (int)((ip.Y- bounds.top) * scale + margin) << "\" r=\"0.1\"/>\n";
            }
          }
        }
      file << "</g>\n";
...
void DoSVG(ClipperData* t, bool showPolyVertices = true, int polyVerticesType = 0);
